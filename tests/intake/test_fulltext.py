...
from distill.intake.services import _REQUEST_DELAY
...
class TestFetchFullText:
    ...
    @patch("distill.intake.services._trafilatura_available", return_value=False)
    ...
    @patch("distill.intake.services.urlopen")
    @patch("distill.intake.services._trafilatura")
    ...
    def test_user_agent_header(self, mock_urlopen_fn: MagicMock) -> None:
        ...
        with patch("distill.intake.services._trafilatura") as mock_traf:
...
class TestEnrichItems:
    ...
    def test_skips_items_with_sufficient_word_count(self) -> None:
        ...
        with patch("distill.intake.services.fetch_full_text") as mock_fetch:
            result = enrich_items(items, min_word_threshold=100)

        mock_fetch.assert_not_called()
        assert len(result) == 2

    @patch("distill.intake.services.time.sleep")
    @patch("distill.intake.services.fetch_full_text")
