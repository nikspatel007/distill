...
class TestCallClaude:
    ...
    def test_nonzero_exit_truncates_long_stderr(self, mock_run: MagicMock) -> None:
        ...
        with pytest.raises(LLMError) as exc_info:
            call_claude("sys", "usr")
        # The message includes stderr[:500], so the error message should have
        # at most 500 'Z' characters from stderr (using Z to avoid overlap
        # with prefix text like "exit")
...
class TestStripJsonFences:
    ...
    def test_lone_brace_no_close(self) -> None:
        ...
        # find("{") returns a position, rfind("}") returns -1
        # so end > start is False => falls through to return text
    ...
    def test_close_before_open_brace(self) -> None:
        ...
        text = "} some text {"
        # rfind("}") == 0, find("{") == 12 => end(0) > start(12) is False
        # Also [ not found, so no candidates => returns text
        # Actually: brace_start=12, bracket_start=-1
        # candidates = [(12, "{", "}")]
        # start=12, end=0 => end(0) > start(12) is False => falls through
