...
# ---------------------------------------------------------------------------
# Lazy re-exports from distill.pipeline.* for backward compatibility.
# Pipeline functions were extracted to keep this module focused on analysis.
# Lazy loading avoids circular imports (pipeline modules import _atomic_write).
# ---------------------------------------------------------------------------
_PIPELINE_EXPORTS: dict[str, tuple[str, str]] = {
    "generate_blog_posts": ("distill.pipeline.blog", "generate_blog_posts"),
    "generate_images": ("distill.pipeline.intake", "generate_images"),
    "generate_intake": ("distill.pipeline.intake", "generate_intake"),
    "generate_journal_notes": ("distill.pipeline.journal", "generate_journal_notes"),
    "DAILY_SOCIAL_STATE_FILENAME": ("distill.pipeline.social", "DAILY_SOCIAL_STATE_FILENAME"),
    "DailySocialState": ("distill.pipeline.social", "DailySocialState"),
    "generate_daily_social": ("distill.pipeline.social", "generate_daily_social"),
}


def __getattr__(name: str) -> Any:
    if name in _PIPELINE_EXPORTS:
        module_path, attr = _PIPELINE_EXPORTS[name]
        import importlib
